...
def train(epoch, log_interval=100):
    ...
    for ep in range(epoch):
        ...
        for batch_idx, (data, target) in enumerate(train_set_loader):
            print(f'@{batch_idx}_start: {p1.requires_grad}, {p1.grad}')
            ...
            print(f'@{batch_idx}_loss: {p1.requires_grad}, {p1.grad}')
            # backward pass
            ...
            print(f'@{batch_idx}_after_zero_grad: {p1.requires_grad}, {p1.grad}')
            # accumulate (i.e. add) the gradients from this forward pass
            loss.backward()  # Any argument is not needed since loss is scalar tensor.
            print(f'@{batch_idx}_after_backward: {p1.requires_grad}, {p1.grad}')
            # performs a single optimization step (parameter update)
            optimizer.step()
            print(f'@{batch_idx}_after_step: {p1.requires_grad}, {p1.grad}')
