...
from models.MRN_masked_attention_v2 import MRN4GRU
...
class MRN4Rec(nn.Module):
    ...
    def forward(self, user, item, behavior, mask_len):
        ...
        0. core의 attension 및 다른 hidden state를 추가로 concat해서 함께 출력하는 방법 생각하기 + 이전 core 상태 활용?
        :param
        ...
        user_embedding = self.user_embedding(user)  # (B, H)
        core = self.mrn(user_embedding, item_embedding, behavior, mask_len)  # (B, H)
